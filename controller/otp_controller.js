...
const sendotp = async (req, res) => {
    try {
        ...
        // Check if the phone number is provided
        if (!phone) {
            return { success: false, status: 400, msg: 'Phone number is required' };
        }
        ...
        // Respond with an error message
        return { success: false, status: 500, msg: 'Error in sending OTP' };
    }
};
...
const verifyotp = async (req, res) => {
    try {
        ...
        // Check if phone number and OTP are provided
        if (!phone || !otp) {
            return { success: false, status: 400, msg: 'Phone number and OTP are required' };
        }
        ...
        if (!otpRecord) {
            return { success: false, status: 404, msg: 'Phone number not found' };
        }
        ...
        // Check if the OTP matches
        if (storedOtp !== otp) {
            return { success: false, status: 400, msg: 'Incorrect OTP' };
        }

        // Check if the OTP has expired
        if (new Date() > otpExpiration) {
            return { success: false, status: 400, msg: 'OTP expired' };
        }
